...
`PYTHONPATH=$MONOREPO_ROOT/.agents/skills/dev-archive/scripts python3 -m dev_archive check-diff --root "$ROOT_REPO"`
...
`PYTHONPATH=$MONOREPO_ROOT/.agents/skills/dev-archive/scripts python3 -m dev_archive ensure-label --root "$ROOT_REPO"` - creates `docs` label if not exists.

### Step 4: Create PR

`PYTHONPATH=$MONOREPO_ROOT/.agents/skills/dev-archive/scripts python3 -m dev_archive create-pr --root "$ROOT_REPO" --title "$TITLE" --body "$BODY"`

Returns `url` and `pr` number.

### Step 5: Squash merge

`PYTHONPATH=$MONOREPO_ROOT/.agents/skills/dev-archive/scripts python3 -m dev_archive squash-merge --root "$ROOT_REPO" --pr $PR`

Squash-merges the PR without deleting the `docs` branch.

### Step 6: Sync branch

`PYTHONPATH=$MONOREPO_ROOT/.agents/skills/dev-archive/scripts python3 -m dev_archive sync-branch --root "$ROOT_REPO"`
