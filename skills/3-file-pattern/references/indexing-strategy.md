...
- **빠른 스캔**: tasks/ 폴더 목록이나 인덱스만 읽어 현재 작업과 관련된 항목 신속 파악
- **유지보수성**: 새 항목 추가 시 하위 파일만 생성하고 인덱스에 한 줄 추가
- **독립적 관리**: 각 task가 독립 파일이므로 병렬 작업, 재정렬, 삭제가 용이

## 파일 읽기 규칙

### tasks/ 폴더
1. **폴더 목록 먼저 확인**: `tasks/` 디렉토리의 파일 목록을 먼저 확인 (ls 또는 Glob)
2. **관련 task만 읽기**: 파일명에서 topic을 보고 현재 작업과 관련된 task 파일만 읽기
3. **전체 읽기 금지**: tasks/ 디렉토리의 모든 파일을 한 번에 읽지 마라 (deep 모드 제외)

### findings/, progress/ 인덱스
1. **항상 인덱스 먼저 읽기**: `findings.index.md`, `progress.index.md`를 먼저 읽어라
2. **선택적 하위 파일 읽기**: 인덱스에서 현재 작업과 관련된 키워드/태그를 찾아 해당 하위 파일만 읽어라
3. **전체 읽기 금지**: findings/, progress/ 디렉토리의 모든 파일을 한 번에 읽지 마라
...
2. findings.index.md에서 해당 키워드가 포함된 항목 찾기
3. 찾은 항목의 하위 파일만 읽기
...
### task 파일
task 파일 생성 전 항상 다음을 실행:
1. tasks/ 디렉토리 리스트 확인
2. 파일명에서 순번 추출 (예: task-05-test-taxonomy.md → 05)
3. 최대 순번 + 1로 새 파일 생성

### findings 파일
findings 파일 생성 전 항상 다음을 실행:
1. findings/ 디렉토리 리스트 확인
