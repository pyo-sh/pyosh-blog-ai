...
- OAuth 로그인은 방명록/댓글 사용자 전용. 후속 버전에서 지원 (관리자에는 적용하지 않음)
...
| 16 | 글 관리 (목록, 필터, 삭제/복원) |
| 17 | 글 에디터 (마크다운 + 프리뷰) |
| 18 | 카테고리 관리 (트리, CRUD, 순서) |
| 19 | 에셋 라이브러리 (업로드, 갤러리, 삭제) |
| 20 | 댓글 관리 (목록, 비밀글 확인, 강제 삭제) |
| 21 | 방명록 관리 (목록, 강제 삭제) |
...
| # | 기능 |
|---|---|
| 22 | 메타태그 + OG + sitemap + RSS |
| 23 | robots.txt + Canonical URL |
| 24 | 구조화 데이터 (JSON-LD) |
| 25 | Favicon / Web Manifest |
...
1. **OAuth 소셜 로그인** - 방명록/댓글 사용자 전용. 서버 구현 완료, v1 클라이언트 미지원 (심사 필요). 관리자에는 적용하지 않음
...
1. **OAuth 소셜 로그인** (Google/GitHub) - 방명록/댓글 사용자 전용. 서버 구현 완료, 클라이언트 연동 + 심사
