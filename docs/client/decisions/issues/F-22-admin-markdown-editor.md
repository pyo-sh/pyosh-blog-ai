...
| 5 | H2 | H2 | `## ` | - | 줄 앞에 삽입/토글 |
| 6 | H3 | H3 | `### ` | - | 줄 앞에 삽입/토글 |
| 7 | 링크 | 링크 | `[text](url)` | Ctrl+K | 선택 텍스트 → 링크 |
| 8 | 이미지 | 이미지 | `![alt](url)` | - | 파일 선택 트리거 |
...
- [ ] 이미지 파일 검증 실패 시 토스트 에러 메시지가 표시되고 삽입이 중단된다
- [ ] 저장 시 이미지 업로드 실패 시 에러 메시지가 표시되고 저장이 중단된다
- [ ] 대기 중 이미지 개수가 표시된다
