...
> 관리자 글 관리 페이지. 글 목록을 테이블 형태로 표시하며, 상태/공개여부/삭제 필터, 검색, 정렬, 벌크 선택/삭제, 공개여부 인라인 토글, 글 미리보기, 영구 삭제(연쇄 삭제 포함)를 제공한다.

## SPEC 참조

- `docs/client/specs/admin-post-list.md`

## 와이어프레임

- `docs/client/designs/admin/admin-post-list.html` - 글 관리 목록 페이지
...
│  [활성 글]  [휴지통 (3)]                              │
│                                                      │
│  상태: [전체 ▼]  공개: [전체 ▼]  [검색어 입력___] [검색] │
│                                                      │
│  선택된 2개: [일괄 삭제]                               │
│                                                      │
│  체크  고정  제목    상태  공개  조회 댓글 작성일 ...  │
│  [v]   고정  Next.js   발행  [on]  245   12   03.20  ...  │
│  [v]   .   React     초안  [off]   0     0   03.18  ...  │
│  [ ]   .   CSS 팁    발행  [on]  189    5   03.15  ...  │
│                                                      │
...
```
[검색어 입력_______________] [검색]
```

- 입력 후 Enter 또는 검색 클릭 시 검색 실행
...
- 클릭 2회: 해당 컬럼 오름차순
- 클릭 3회: 기본 정렬로 복귀
- 헤더에 현재 정렬 방향 화살표 표시, 미정렬 시 양방향 화살표
...
| 탭 | 가능한 일괄 작업 |
|---|---|
| 활성 글 | 카테고리 변경, 댓글 상태 변경, 일괄 삭제 |
| 휴지통 | [일괄 복원], [일괄 영구 삭제] |

#### 활성 글 - 벌크 액션 바

체크박스 선택 시 상단에 액션 바가 나타난다.

```
┌─────────────────────────────────────────────────────────────────────┐
│ 전체 선택  선택됨 3개  카테고리: [- ▼]  댓글: [- ▼]  [초기화] [적용] [삭제] │
└─────────────────────────────────────────────────────────────────────┘
```

| 요소 | 동작 |
|---|---|
| 전체 선택 | 현재 페이지 전체 선택/해제 토글 |
| 선택됨 N개 | 선택 카운터 |
| 카테고리: [- ▼] | 변경할 카테고리 선택. F-23 트리 드롭다운(인덴트 표기) 재사용. 기본 `-`: 변경 안 함 |
| 댓글: [- ▼] | 변경할 댓글 상태 선택 (열림/잠김/비활성). 기본 `-`: 변경 안 함 |
| [초기화] | 드롭다운 선택 초기화 (모두 `-`로 복귀) |
| [적용] | 변경사항 확인 모달 → 서버 전송. 드롭다운 하나 이상 변경 시만 활성 |
| [삭제] | 일괄 소프트 삭제 확인 모달 |

#### 통합 확인 모달

[적용] 클릭 시 모든 변경사항을 한번에 확인한다.
...
- 변경된 항목만 표시 (카테고리만 바꿨으면 카테고리만)
- 댓글 상태별 설명: 열림 "댓글 작성과 표시가 정상 동작합니다.", 잠김 "기존 댓글은 유지되며 새 댓글 작성이 차단됩니다.", 비활성 "댓글 영역이 완전히 숨겨집니다."
- [적용] → 단일 벌크 API 호출 → 성공 토스트 + 목록 갱신 + 선택 해제
...
interface BulkPostAction {
  ...
  action:
    ...
    | "restore"
    | "hard_delete";
  categoryId?: number;           // update 시 (선택)
  commentStatus?: "open" | "locked" | "disabled";  // update 시 (선택)
}
```

- 단일 트랜잭션: 전체 성공 or 전체 실패
- 네트워크 1회 (개별 `Promise.allSettled` 방식 대체)
- `update`: categoryId와 commentStatus를 한번에 변경 가능 (변경된 필드만 전송)
- `hard_delete`: 연쇄 삭제 포함 (아래 영구 삭제 참조)
...
- 관련 글: 카테고리가 설정된 글에 한해 표시 - 카테고리 미설정 시 관련 글 컴포넌트 미렌더링 (API 미호출)
...
- [ ] 활성 글 탭에서 벌크 카테고리 변경이 확인 모달 후 동작한다
- [ ] 활성 글 탭에서 벌크 댓글 상태(commentStatus) 변경이 확인 모달 후 동작한다
- [ ] 벌크 작업이 서버 벌크 API(`PATCH /api/admin/posts/bulk`)로 트랜잭션 처리된다
- [ ] 벌크 작업 실패 시 에러 상세(글 ID + 사유)가 토스트에 표시된다
...
- [ ] 미리보기 컨트롤 바에서 수정, 삭제, 공개 토글, 고정 토글, 상태 변경, 수정일 관리가 가능하다
