...
블로그 홈 페이지. 최신순으로 발행된 글 목록을 리스트형으로 표시하고, 카테고리 필터와 고정 글 기능을 제공한다.

## 2. 배경 및 동기

블로그의 첫 화면으로, 방문자가 최신 글을 빠르게 탐색할 수 있어야 한다. 리스트형 레이아웃으로 정보 밀도를 높이고, 카테고리 필터로 관심 분야를 바로 찾을 수 있게 한다.

## 3. 목표

- 방문자가 최신 글을 빠르게 훑어볼 수 있다
- 카테고리별 필터링으로 원하는 글을 바로 찾는다
...
1. 방문자가 `/` 접속
2. 최신순 글 목록 + 카테고리 필터 드롭박스 + 고정 글 표시
3. 카테고리 드롭박스 선택 → 해당 카테고리 글만 필터링 (URL 변경, reload 없음)
4. 페이지 번호 클릭 → URL 변경, reload 없이 글 목록 교체
5. 글 클릭 → `/posts/{slug}` 이동
...
- 목록 상단에 드롭박스(select) 형태
- 기본값 "전체", 펼치면 카테고리 목록 표시
- 선택 시 URL 변경 (예: `/?category={slug}`), reload 없음
- 페이지 번호 초기화 (1페이지로)
...
Server Component (SSR)
  └─ fetchPosts({ page, category }) → initialData
       └─ Client Component
            └─ useQuery({ queryKey: ["posts", page, category], initialData })
            └─ 페이지/카테고리 변경 시 router.push → queryKey 변경 → refetch (reload 없음)
```

- SSR `initialData` + TanStack Query 패턴
- URL 쿼리 파라미터로 상태 관리: `?page=N&category=slug`
...
| `features` | `PostList` | 글 목록 + 페이지네이션 + 카테고리 드롭박스 |
...
- [ ] 카테고리 드롭박스 선택 시 reload 없이 해당 카테고리 글만 표시된다
- [ ] 페이지네이션: `[<<-5] [<-1] 번호 [+1>] [+5>>]` 형태, 중간 생략(...) 패턴
- [ ] 페이지/카테고리 전환 시 URL이 `?page=N&category=slug`로 변경된다 (reload 없음)
- [ ] 브라우저 뒤로가기 시 이전 페이지/카테고리 상태가 복원된다
- [ ] 범위 초과 페이지 접근 시 404
- [ ] 글이 없을 때 빈 상태 메시지 표시
- [ ] 페이지 전환 시 스켈레톤 로딩 표시
- [ ] 접근성: 키보드로 드롭박스/페이지네이션 조작 가능 (A-01 참조)
...
| 카테고리 필터 + 고정 글 | 해당 카테고리에 속한 고정 글만 상단 표시 |
| 페이지 파라미터가 음수/문자열 | 404 |
| 존재하지 않는 카테고리 slug | 빈 목록 표시 (드롭박스이므로 발생 가능성 낮음, 관리자 삭제 시 대비) |
