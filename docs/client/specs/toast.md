...
전역 Toast 알림 시스템. 서버 요청 결과(성공/실패), 클립보드 복사, 권한 에러 등 사용자 액션에 대한 피드백을 상단 우측 Toast로 표시한다. `sonner` 라이브러리를 사용한다.

## 2. 배경 및 동기

현재 모든 피드백이 인라인 메시지로 처리되어 있어 일관성이 부족하고, `getErrorMessage()` 같은 유틸이 7개 파일에서 중복 정의되어 있다. Toast 시스템을 도입하여 서버 응답 피드백을 통일하고, 공통 유틸을 추출한다.

## 3. 목표

- `sonner` 기반 전역 Toast 시스템을 구축한다
- 서버 요청 결과(성공/실패)를 Toast로 표시한다
- `getErrorMessage()` 유틸을 `@shared/lib`로 추출하여 중복을 제거한다
- Toast 클릭 시 관련 위치로 이동하거나 Toast를 닫는 액션을 지원한다

## 4. 비목표

- 폼 검증 에러 Toast 전환 (인라인 유지)
- 실시간 알림 (WebSocket 기반 push)
- Toast 커스텀 렌더러 (sonner 기본 스타일 활용)
...
- 없음 (기반 기능)
- F-12: 에러 페이지 (403 Toast + 리다이렉트 흐름)
- 서드파티: `sonner`
