...
관리자 에셋 관리 페이지의 이미지 업로드 기능. 드래그 앤 드롭, 파일 선택 버튼, 업로드 큐(미리보기 + 개별 제거), 파일 검증, 전체 진행률 표시를 제공한다. 업로드 시 서버에서 이미지 width/height를 자동 추출하여 DB에 저장한다. 정적 파일에 Cache-Control 헤더를 적용한다.

## 2. 배경 및 동기

현재 구현 상태:

- 드래그 앤 드롭 영역 (`upload-zone.tsx`) + 파일 선택 버튼 (동작)
- 큐 미리보기 + 개별 제거 + 전체 삭제 (동작)
- 파일 검증: JPEG, PNG, GIF, WebP, SVG / 10MB / 최대 5파일 (동작)
- 중복 파일 감지: 파일명 + 크기 + 수정일 기준 (동작)
- `@fastify/multipart` 기반 서버 업로드 (동작)
- 로컬 파일시스템 저장: `uploads/YYYY/MM/{uuid}.{ext}` (동작)
- `@fastify/static`으로 `/uploads/` 경로 서빙 (동작)
- React Query 뮤테이션 + 부분 실패 처리 (동작)

개선이 필요한 부분:

- 업로드 진행률 미표시 (로딩 상태만 표시)
- 이미지 width/height가 DB에 nullable로 있지만 자동 추출 미구현
- 정적 파일 Cache-Control 헤더 미설정

## 3. 목표

- 업로드 시 전체 진행률을 표시한다
- 서버에서 `image-size` 라이브러리로 width/height를 자동 추출하여 DB에 저장한다
- `@fastify/static`에 Cache-Control 헤더를 적용한다
- 기존 구현을 스펙으로 문서화한다

## 4. 비목표

- S3/CDN 스토리지 연동 (v1은 로컬 파일시스템)
- 이미지 리사이즈/WebP 변환/최적화 (v1 범위 밖)
- 이미지 크롭 UI (v1 범위 밖)
- 업로드 취소 (5파일 이하이므로 불필요)
- 개별 파일 진행률 (전체 진행률만 표시)

---

## 5. 상세 설계

### 5.1 업로드 영역 (기존 구현 문서화)
