...
헤더의 검색 버튼으로 검색어를 입력하고, `/search` 페이지에서 검색 결과를 글 목록 형태로 표시한다. 6개 검색 필터(제목, 내용, 제목+내용, 태그, 카테고리, 댓글)를 드롭다운으로 제공하며, 검색어 하이라이팅을 지원한다.

## 2. 배경 및 동기

초기 검색 기능에서 다음 개선을 완료했다:

- SQL `LIKE '%keyword%'` 방식으로 인덱스 미사용, 성능 한계
- 검색 대상이 제목+본문으로 고정 (필터 없음)
- 검색 결과를 메인 페이지와 동일한 리스트 형식으로 변경
- 검색어 하이라이팅 없음
- 태그, 카테고리, 댓글 검색 미지원

## 3. 목표

- MySQL FULLTEXT + ngram 파서로 제목/내용 검색을 전환한다
- 태그/카테고리/댓글 검색을 JOIN + LIKE로 추가한다
- 6개 검색 필터를 드롭다운으로 제공한다
- 검색 결과를 메인 페이지와 동일한 리스트 형식으로 표시한다
- 클라이언트 검색어 하이라이팅을 추가한다
- 헤더 검색 입력 UX를 개선한다
