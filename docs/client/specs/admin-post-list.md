...
- 일괄 삭제/복원: 선택된 글 ID 배열을 순차 또는 병렬로 처리
- 일괄 영구 삭제: 확인 다이얼로그 필수 ("N개의 글과 관련 댓글이 영구 삭제됩니다. 이 작업은 되돌릴 수 없습니다.")
- 처리 중 프로그레스 표시 ("3/5 삭제 중...")

#### 서버 변경

벌크 API를 추가하거나, 클라이언트에서 개별 API를 `Promise.allSettled`로 병렬 호출. v1에서는 클라이언트 병렬 호출 방식 사용.
...
| 벌크 삭제 중 일부 실패 | `Promise.allSettled` 사용, 성공/실패 건수 표시 |
