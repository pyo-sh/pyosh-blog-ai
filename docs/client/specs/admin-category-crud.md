...
관리자 카테고리 관리 페이지의 생성/수정/삭제 모달. 이름, 부모 카테고리, 공개여부를 폼으로 관리하고, 삭제 시 해당 카테고리에 속한 글을 다른 카테고리로 이동하거나 휴지통으로 이동하는 선택지를 제공한다. F-24 트리 표시 위에서 동작한다.

## 2. 배경 및 동기

현재 구현 상태:

- 생성/수정 모달: 이름, 부모 선택, 공개여부 체크박스 (동작)
- 삭제 모달: 확인 대화상자 (동작)
- 순환참조 방지: 부모 드롭다운에서 자기 자신 + 자손 제외 (동작)
- 삭제 검증: 하위 카테고리 존재 시 클라이언트 차단, 글 존재 시 서버 409 (동작)
- React Query 뮤테이션 + 캐시 무효화 (동작)

개선이 필요한 부분:

- 글이 있는 카테고리 삭제가 불가 (글 이동/휴지통 옵션 없음)
- 삭제 시 글 처리에 대한 사용자 선택지 없음

## 3. 목표

- 카테고리 삭제 시 글 처리 옵션을 제공한다 (다른 카테고리로 이동 / 휴지통으로 이동)
- 삭제 API를 확장하여 글 처리를 트랜잭션으로 수행한다
- 기존 생성/수정 모달의 동작을 유지하고 스펙으로 문서화한다

## 4. 비목표

- 카테고리 순서 변경 (F-24 배치 편집 모드에서 처리)
- 카테고리 일괄 공개/숨김 변경 (F-24 일괄 선택 모드에서 처리)
- 카테고리 부모 변경 via 드래그 (F-24 배치 편집 모드에서 처리)
- 인라인 이름 편집
- 카테고리 설명/아이콘 필드 (v1 범위 밖)
...
- 기본값: 빈 상태 (미선택)
- 미선택 상태에서 [삭제] 클릭 시 "이동할 카테고리를 선택해 주세요" 에러 메시지 표시
...
F-21의 글 목록에서 벌크 카테고리 변경이 가능하다. `PATCH /api/admin/posts/bulk` 엔드포인트를 사용하며, 상세는 F-21 스펙 5.5절 참조.
...
- F-24 카테고리 트리 표시 (트리 UI, 수정/삭제 버튼 트리거)
- F-19 관리자 로그인 (인증)
