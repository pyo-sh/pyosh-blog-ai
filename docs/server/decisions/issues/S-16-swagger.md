...
> 모든 스키마에 예시/설명 추가, 인증 선언, CSRF/rate-limit 문서화, 태그 확장, 파일 업로드 수정, 프로덕션 비활성화

## SPEC 참조

- `docs/server/specs/swagger-docs.md` > 전체
...
- Blocked by: S-04~S-12 (모든 라우트)
- Blocks: 없음

## 참고

- `zod-openapi` 패키지 추가 필요 여부 확인: `.openapi({ example })` 사용 시 별도 패키지가 필요할 수 있다. `.describe()`만으로 충분한지, 스키마 전체 example이 필요한지 구현 단계에서 결정한다.
- Zod `.transform()` 후 스키마는 OpenAPI에 반영되지 않을 수 있으므로, 응답 스키마에서 변환 후 타입을 명시한다.
